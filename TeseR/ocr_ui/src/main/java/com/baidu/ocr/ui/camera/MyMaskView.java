...
public class MyMaskView extends View {
...
//            return new Rect(getWidth()/2, getHeight()/2, getWidth()/2, getHeight()/2);
...
//                        R.drawable.bd_ocr_id_card_locator_back, null);
    ...
    public MyMaskView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MyMaskView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            float ratio = h > w ? 0.9f : 0.72f;
            float ratio = h > w ? 0.8f : 0.72f;
            int height = (int) (w * ratio);
            int width = height * 480 / 620;
